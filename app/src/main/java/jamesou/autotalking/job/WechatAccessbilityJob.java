...
import jamesou.autotalking.biz.MessageWrapper;
import jamesou.autotalking.service.AutoTalkingService;
import jamesou.autotalking.utils.AccessibilityHelper;
import jamesou.autotalking.utils.Config;
import jamesou.autotalking.utils.NotifyHelper;
...
public class WechatAccessbilityJob extends BaseAccessbilityJob {
    ...
    public boolean isEnable() {
        return getConfig().isEnableAutoTalkingService();
    ...
    public void onReceiveJob(AccessibilityEvent event) {
        ...
        if(eventType == AccessibilityEvent.TYPE_NOTIFICATION_STATE_CHANGED)
        {
            ...
            openChatWindow((Notification) data);
            if(AutoTalkingService.me==null)
            talk(Config.ROBOT_FIRST_FEEDBACK);
        }else if(eventType == AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED&&"com.tencent.mm.ui.LauncherUI".equals(event.getClassName()))
        {
            if(AutoTalkingService.me==null)
            talk(Config.ROBOT_FIRST_FEEDBACK);
        }
        else if(eventType == AccessibilityEvent.TYPE_WINDOW_CONTENT_CHANGED&&"android.widget.TextView".equals(event.getClassName())) {
            robotFeedback();
        }

         //else if(eventType == AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED) {
           // openHongBao(event);
          //  robotFeedback(event.getSource());
    ...
    public class SendRequestThread extends Thread
    {
        ...
        public SendRequestThread(String chatText)
        {
        ...
        public void run()
        {
            ...
            message.obj= MessageWrapper.getFeedBackText(chatText);
    ...
    @TargetApi(Build.VERSION_CODES.JELLY_BEAN)
    private void openChatWindow(Notification notification) {
    ...
    @TargetApi(Build.VERSION_CODES.JELLY_BEAN)
    private void robotFeedback() {
        AccessibilityNodeInfo chatNodeInfo = getService().getRootInActiveWindow();
        AccessibilityNodeInfo userNodeInfo = AccessibilityHelper.findLastNodeInfosById(chatNodeInfo, "com.tencent.mm:id/bc");//最新版微信6.3.16
        AccessibilityNodeInfo chatTextNodeInfo = AccessibilityHelper.findLastNodeInfosById(chatNodeInfo, "com.tencent.mm:id/bf");//最新版微信6.3.16
        if(userNodeInfo!=null&&chatTextNodeInfo!=null) {
           String lastChatUser = null;
           lastChatUser = userNodeInfo.getContentDescription().toString();
           String chatText = chatTextNodeInfo.getText().toString();
           if (AutoTalkingService.me == null&&chatText.equals(Config.ROBOT_FIRST_FEEDBACK)) AutoTalkingService.me = lastChatUser;
           if (lastChatUser != null && !lastChatUser.equals(AutoTalkingService.me)) {
               SendRequestThread sendRequestThread = new SendRequestThread(chatText);
    ...
    private void talk(String text){
        AccessibilityNodeInfo chatNodeInfo = getService().getRootInActiveWindow();
        AccessibilityNodeInfo chatEditText =   AccessibilityHelper.findNodeInfosById(chatNodeInfo, "com.tencent.mm:id/c6v");//最新版微信6.3.16
        if(chatEditText!=null)
            AccessibilityHelper.performSetText(chatEditText, text,getContext());
        AccessibilityNodeInfo sendButton = AccessibilityHelper.findNodeInfosById(chatNodeInfo, "com.tencent.mm:id/c70");//最新版微信6.3.16
        if(sendButton != null) {
            final AccessibilityNodeInfo n = sendButton;
         ...
         //   AccessibilityHelper.performClick(sendButton);//todo延迟一下，解决重复的回复
    }
    @TargetApi(Build.VERSION_CODES.JELLY_BEAN)
